...
//
// Fonte ÚNICA de verdade, consumida pelos dois lados:
//   - Frontend (Vite/ESM): import ... from ".../permissions.mjs"
//   - Backend/Electron (CJS): require(".../permissions.js")
// O conteúdo dos dois arquivos deve ser mantido idêntico (paridade).
// =============================================================
...
// Lista achatada de todas as capabilities (ordem estável).
export const ALL_CAPABILITIES = PERMISSION_MODULES.flatMap((m) => m.caps.map((c) => c.key));

const capsFor = (...keys) => keys;

// Presets por cargo (templates iniciais; editáveis na UI depois).
// `admin` recebe tudo dinamicamente em computeEffectivePermissions.
 ...
 * effective = base(cargo|roleTemplate) ∪ grants − denies.
 * `admin` sempre recebe tudo (não pode ser bloqueado por override).
 *
 * @param {{ cargo?: string, roleTemplate?: string[]|null, overrides?: { grants?: string[], denies?: string[] } }} input
 * @returns {string[]} capabilities efetivas (subconjunto de ALL_CAPABILITIES)
 */
...
/** Testa se uma lista/Set de permissões efetivas contém a capability. */
export function hasCapability(effective, capability) {
...
// Mapa rota (frontend) → capability necessária para acessá-la.
export const ROUTE_CAPABILITY = {
  "/": null, // painel: sempre visível a quem está logado
  "/vendas": "sales.view",
...
export const STORE_ROLES = ["admin", "gerente", "caixa", "vendedor"];
