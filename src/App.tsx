...
// Extensão do objeto window para o IPC do Electron
declare global {
  interface Window {
    api: any;
  }
...
function App() {
  ...
  useEffect(() => {
    ...
    const handleAvailable = (ver: string) => {
    ...
    const handleProgress = (p: number) => setDownloadProgress(p);
  ...
  if (!user) {
    return <Login onLoginSuccess={(userData: User) => setUser(userData)} />;
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100 font-sans text-gray-900">
      <aside className="w-64 bg-brand-deep text-white flex flex-col flex-shrink-0 transition-all duration-300 shadow-2xl z-10">
        ...
        <div className="px-4 py-4 bg-black/5 border-b border-white/5 flex items-center justify-between">
          ...
          <div className="bg-white rounded-xl shadow-2xl p-8 w-96 max-w-[90%] transform transition-all scale-100 border border-gray-200">
            ...
            <form onSubmit={handleSupervisorAuth} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isAuthLoading}
                className={`w-full py-3 rounded-xl font-bold transition shadow-lg flex justify-center items-center ${
                  isAuthLoading
