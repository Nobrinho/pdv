...
import { Sale, Person, SalePayment } from "../types";

dayjs.locale("pt-br");

interface SaleWithCalculated extends Sale {
  comissao_calculada: number;
}

interface LaborSummaryItem {
  nome: string;
  total: number;
  qtd: number;
}
...
const Relatorios: React.FC = () => {
  ...
  const standardizeMethod = (method?: string | null): string => {
  ...
  const [filteredSales, setFilteredSales] = useState<SaleWithCalculated[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      setAllSales(Array.isArray(sales) ? sales.sort((a, b) => dayjs(b.data_venda).valueOf() - dayjs(a.data_venda).valueOf()) : []);
      setAllServices(Array.isArray(services) ? services.sort((a, b) => dayjs(b.data_servico).valueOf() - dayjs(a.data_servico).valueOf()) : []);
      setAllPeople(people || []);
  ...
  const isWithinRange = (timestamp: number) => {
    ...
    return (
      dateToCheck.isSame(start, "day") ||
      dateToCheck.isSame(end, "day") ||
  ...
  const processData = () => {
    ...
    let vendasFiltradas = allSales.filter((s) => {
      const inDate = isWithinRange(s.data_venda || 0);
      const isSeller =
    ...
    const addPaymentToMap = (metodoRaw: string | undefined | null, valor: number) => {
    ...
    const vendasProcessadas: SaleWithCalculated[] = vendasFiltradas.map((venda) => {
      ...
      const custoReal = (venda as any).custo_total_real || 0; // custo_total_real needed in Sale type?
      ...
      // 2. Fallback: Se o backend não mandou (versão antiga ou venda sem itens), calcula estimativa no Front
        ...
        const taxa = vendedor?.comissao_fixa !== undefined
    ...
    const mapMO: Record<number, LaborSummaryItem> = {};
    vendasFiltradas.forEach((v) => {
      if (!v.cancelada && v.mao_de_obra > 0 && v.trocador_id) {
        if (!mapMO[v.trocador_id])
          mapMO[v.trocador_id] = { nome: v.trocador_nome || "Desconhecido", total: 0, qtd: 0 };
  ...
  const exportPDF = () => {
    try {
      ...
      const tableData: any[][] = filteredSales.map((v) => {
        ...
        return [
          ...
          v.acrescimo_valor > 0 ? formatCurrency(v.acrescimo_valor) : "-",
