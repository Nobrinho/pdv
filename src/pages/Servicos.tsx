...
const Servicos: React.FC = () => {
  ...
  const [mechanics, setMechanics] = useState<Person[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const sortedServices = (servicesData || []).sort(
        (a: Service, b: Service) => (b.data_servico || 0) - (a.data_servico || 0),
      );

      setServices(sortedServices);

      // Filtra apenas quem tem cargo de "Trocador"
      setMechanics((peopleData || []).filter((p: Person) => p.cargo_nome === "Trocador"));
    } catch (error) {
