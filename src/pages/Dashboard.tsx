...
const Dashboard: React.FC = () => {
  ...
  const [chartData, setChartData] = useState<WeeklySales & { datasets: any[] }>({ labels: [], data: [], datasets: [] });
  ...
  const loadDashboardData = async () => {
    try {
          ...
          datasets: [
            {
              label: "Faturamento (R$)",
              data: weeklyData.data,
              backgroundColor: "#2563eb99", // brand-primary com transparência
              borderRadius: 4,
            },
          ],
        });
      ...
      if (invStats) setInventoryStats(invStats);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <div className="p-6 h-full flex flex-col overflow-y-auto bg-gray-50 dark:bg-slate-950">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-gradient-to-br from-brand-deep to-brand-dark p-4 rounded-xl shadow-md text-white">
